...
function clear(event) /*новая*/{
    const list = document.getElementById('autocomplete-list');
    const cell = document.querySelector('.search-field');
    if (event && event.target === cell) return; 
    list.innerHTML = '';
}

function autocomplete(event, arr)/*новая*/ {
    const autofillList = document.getElementById('autocomplete-list');
    ...
    for (const phrase of arr) {
        ...
        item.innerHTML = "<strong>" + phrase.substr(0, value.length) 
                                                        + "</strong>";
        ...
        autofillList.append(item);
    }
}

function autofillItems(event) /*новая*/ {
